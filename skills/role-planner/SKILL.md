...
2. **Gather implementation context.** Inspect only the repo files needed to
   understand likely scope, existing patterns, test commands, and risk. Prefer
   existing architecture and conventions over new abstractions.

3. **Copy hard requirements forward.** Put established requirements near the top
...
4. **Split into runnable phases.** Break the plan into phases when expected work
...
5. **Make completion checkable.** Every phase must have an unchecked checkbox.
   Each phase should end with the app runnable and relevant tests passable when
   feasible.

6. **Include validation expectations.** Name the kinds of unit tests,
...
7. **Stage the plan.** After writing `plan.md`, stage it with `git add plan.md`.
